"""
US-MOD-02: Claim next ticket from moderation queue.
...
class GetNextTicketTests(APITestCase):
    ...
    def test_next_returns_oldest_pending(self):
        ...
        Response contains id, kind, status=IN_REVIEW, queue_priority, created_at.
        """
        ...
        self.assertEqual(str(resp.data["id"]), str(old_ticket.id))
        self.assertEqual(resp.data["kind"], TicketKind.CREATE)
    ...
    def test_specific_queue_priority_respected(self):
        """queue_priority=2 returns only from queue 2, even if queue 1 has tickets."""
        q1_ticket = _make_pending_ticket(queue_priority=1)
        q2_ticket = _make_pending_ticket(queue_priority=2)

        resp = self.client.post(URL, {"queue_priority": 2}, format="json")

        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(str(resp.data["id"]), str(q2_ticket.id))
        q1_ticket.refresh_from_db()
        self.assertEqual(q1_ticket.status, TicketStatus.PENDING)

    def test_auto_priority_falls_through_to_next_queue(self):
        """No queue_priority → tries 1, 2, 3, 4; returns from first non-empty."""
        q3_ticket = _make_pending_ticket(queue_priority=3)

        resp = self.client.post(URL, {}, format="json")

        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(str(resp.data["id"]), str(q3_ticket.id))
    ...
    def test_empty_specific_queue_returns_204(self):
        ...
        resp = self.client.post(URL, {"queue_priority": 1}, format="json")
        self.assertEqual(resp.status_code, status.HTTP_204_NO_CONTENT)
    ...
    # ── invalid queue_priority ─────────────────────────────────────────────────

    def test_invalid_queue_priority_returns_400(self):
        resp = self.client.post(URL, {"queue_priority": 5}, format="json")
